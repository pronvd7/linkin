 ...
 export async function getPageDatawLinkAndSocialData(username = '') {
  ...
  try {
    ...
    if(typeof pageData != 'undefined' && pageData.username === username){
      ...
      let notificationsData = await getNotificationData(pageData.id);
      return { success: true, pageData: pageData, ...linkData, ...socialData, ...mediaData, ...notificationsData };
    }else{
      return { success: false, pageData: [], linkData: [], socialData:[], mediaData: [], notificationsData:[] };
    }
  } catch (error) {
    throw new Error(error.message);
  }
 ...
 * @returns
 */
 export async function getNotificationData(pagedataid = null) {
  try {
    let notificationsData = [];
    if(pagedataid){
      let notificationsDataResponse = await Prisma.notificationsdata.findMany({
        orderBy: { orderIndex: "asc" },
        where: {pagedataid : pagedataid},
      });

      notificationsDataResponse.forEach((element, index) => {
        let { created_at, ...notificationsDataRows } = element;
        notificationsData.push(notificationsDataRows);
      });
    }
 ...
 */
 export async function insertNotification(data) {
  ...
  try {
    let insertNotificationsdata = Prisma.notificationsdata.create({ data });

    return insertNotificationsdata;
  } catch (error) {
    throw new Error(error.message);
  }
}
 ...
 */
 export async function updateNotification(data) {
  try {
    ...
    let updatedNotifications = await Prisma.notificationsdata.update({
      data: dataWOid,
      where: { id },
    });
 ...
 */
 export async function deleteNotification({ id }) {
  ...
  try {
    let response = await Prisma.notificationsdata.delete({ where: { id } });
 ...
 * @returns
 */
 export async function reorderNotifications(data = []) {
  if (data.length < 1) {
    throw new Error("invalid data");
  }
  ...
  try {
    data.forEach((notification) => {
      promiseArr.push(
        Prisma.notificationsdata.update({
          where: { id: notification.id },
          data: { orderIndex: notification.orderIndex },
        })
      );
    });
